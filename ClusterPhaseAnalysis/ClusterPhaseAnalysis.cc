    ...
    for (const auto& [trackKey, track] : *trackmap)
    {
        ...
        if (checkTrack(track))
        {
           // if(!isSimulation) 
            ...
            ComputeCircularFitFromNeighbors(track);  
            FillClusters(track);
...
//_____________________________________________________________________________________________
void ClusterPhaseAnalysis::ComputeCircularFitFromNeighbors(SvtxTrack* track)
{
  if (!track || !geometry || !clustermap || !tpcGeom) return;
  ...
  // Get track curvature for circular fit
  // κ = q/(pT·c) × B, with c=1 in natural units, B in Tesla
  const double charge = track->get_charge();
  const double pt = track->get_pt();
  const double B_field = 1.4; // Tesla - adjust to your actual field strength
  
  double kappa = 0.0;
  if (std::abs(pt) > 1e-6)
  {
    kappa = charge * 0.3 * B_field / pt; // 0.3 is the conversion factor (GeV/c)/(T·cm)
  }
  ...
  for (auto& layerPair : clustersByLayer)
  {
    ...
    const double phiErrCurrent = sqrt(para_errors.first) / rCurrent;  // radians
    ...
    const double phiErrMinus1 = sqrt(para_errors_Minus1.first) / rMinus1 ;  // radians
    ...
    const double phiErrPlus1 = sqrt(para_errors_Plus1.first) / rPlus1;  // radians
    ...
    // Method 1: Exact circular arc through 3 points
    // For a circle in the r-φ plane with center at (r_c, φ_c) and radius R_circle
    // We'll use a simpler approach: parabolic correction to linear fit
    
    // First, compute linear interpolation
    const double slope_linear = (phiPlus1 - phiMinus1) / (rPlus1 - rMinus1);
    const double intercept_linear = phiMinus1 - slope_linear * rMinus1;
    const double phiFit_linear = intercept_linear + slope_linear * rCurrent;
    
    // Circular correction using track curvature
    // For circular trajectory: Δφ ≈ linear_term + quadratic_term
    // The quadratic correction accounts for the arc
    
    double phiFit = phiFit_linear;
    const int circular_mode = 1; 
    if (circular_mode==2 && std::abs(kappa) > 1e-9) // Only apply correction if curvature is significant
    {

      // Distance from middle point to neighbors
      const double dr_minus = rCurrent - rMinus1;
      const double dr_plus = rPlus1 - rCurrent;
      const double dr_total = rPlus1 - rMinus1;
      
      // For a circular arc, the second-order correction is:
      // Δφ_correction = κ² × (dr_minus × dr_plus) / (2 × (1 + small_terms))
      // This is the difference between circular arc and straight line
      
      // Simplified version (valid for small κr):
      const double curvature_correction = 0.5 * kappa * kappa * dr_minus * dr_plus / dr_total;
      
      // More accurate version using exact circular geometry:
      // s_minus = arc length from r_minus to r_current
      // s_plus = arc length from r_current to r_plus
      // For small angles, s ≈ sqrt(dr² + (r·dφ)²) ≈ r·dφ
      
      // Alternative: use exact 3-point circle fit
      // Fit a circle through the 3 points in Cartesian coordinates
      const double x1 = rMinus1 * std::cos(phiMinus1);
      const double y1 = rMinus1 * std::sin(phiMinus1);
      const double x2 = rCurrent * std::cos(phiFit_linear);  // use linear estimate
      const double y2 = rCurrent * std::sin(phiFit_linear);
      const double x3 = rPlus1 * std::cos(phiPlus1);
      const double y3 = rPlus1 * std::sin(phiPlus1);
      
      // Circle center calculation
      const double A = x1 - x2;
      const double B = y1 - y2;
      const double C = x1 - x3;
      const double D = y1 - y3;
      const double E = (x1*x1 - x2*x2 + y1*y1 - y2*y2) / 2.0;
      const double F = (x1*x1 - x3*x3 + y1*y1 - y3*y3) / 2.0;
      
      const double det = A * D - B * C;
      
      if (std::abs(det) > 1e-9)
      {
        const double x_center = (E * D - B * F) / det;
        const double y_center = (A * F - E * C) / det;
        
        // Now find phi at rCurrent on this circle
        // The point (x,y) at radius rCurrent satisfies:
        // (x - x_center)² + (y - y_center)² = R_circle²
        // x² + y² = rCurrent²
        
        const double R_circle_sq = (x1 - x_center) * (x1 - x_center) + 
                                   (y1 - y_center) * (y1 - y_center);
        const double R_circle = std::sqrt(R_circle_sq);
        
        // Solve for the intersection
        const double a_coef = 1.0 + (x_center * x_center + y_center * y_center - R_circle_sq) / (rCurrent * rCurrent);
        
        if (a_coef >= 0)
        {
          // Use the angular difference as correction
          const double phi_at_r = std::atan2(y_center, x_center);
          const double opening_angle = std::acos(std::min(1.0, std::max(-1.0, 
            (rCurrent * rCurrent + x_center * x_center + y_center * y_center - R_circle_sq) / 
            (2.0 * rCurrent * std::sqrt(x_center * x_center + y_center * y_center)))));
          
          // Choose the solution closer to linear prediction
          const double phi_solution_1 = phi_at_r + opening_angle;
          const double phi_solution_2 = phi_at_r - opening_angle;
          
          const double diff1 = std::abs(std::remainder(phi_solution_1 - phiFit_linear, 2.0 * M_PI));
          const double diff2 = std::abs(std::remainder(phi_solution_2 - phiFit_linear, 2.0 * M_PI));
          
          phiFit = (diff1 < diff2) ? phi_solution_1 : phi_solution_2;
          std::cout<<"ClusterPhaseAnalysis::ComputeCircularFitFromNeighbors -- ckey "<<ckey
                   <<" layer "<<currentLayer<<" using exact circular fit correction."<<std::endl;
          std::cout<<"Circle radius R_circle "<<R_circle
                   <<" center ( "<<x_center<<","<<y_center<<" )"
                   <<" FOR KAPPA "<<kappa<<" 1/kappa "<<1.0/kappa
                   <<std::endl;
          
        }
        else
        {
          // Fallback to simple curvature correction
          phiFit = phiFit_linear + curvature_correction;
        }
      }
      else
      {
        // Points are collinear or calculation failed, use simple correction
        phiFit = phiFit_linear + curvature_correction;
      }
    }
    else if (circular_mode == 1)
    {
      // distances in radius
        const double dr_minus = rCurrent - rMinus1;
        const double dr_plus  = rPlus1  - rCurrent;

        // Track radius of curvature in the xy-plane
        // kappa = q*0.3*B/pT is in 1/m (standard HEP convention), so convert to cm:
        const double kappa_cm = kappa / 100.0;                 // 1/cm
        const double rho_cm   = 1.0 / std::abs(kappa_cm);      // cm

        // Sign of bending (assumes Bz>0; if your Bz can be negative, include its sign)
        const double bend_sign = (kappa_cm >= 0.0) ? +1.0 : -1.0;

        // --- “Method 1” small-angle curvature correction ---
        // This is a local second-order correction to linear interpolation.
        // Dimensionless (radians) because dr^2 / (rho * r) is dimensionless.
        const double curvature_correction =
            bend_sign * 0.125 * (dr_minus * dr_plus) / (rho_cm * rCurrent);  // ~Δr^2/(8 ρ r)

        phiFit = phiFit_linear + curvature_correction;

        if (Verbosity() > 0)
        {
          std::cout << "ComputeCircularFitFromNeighbors -- using Method1 only: "
                    << "rho_cm=" << rho_cm
                    << " corr=" << curvature_correction
                    << std::endl;
        }
    }
     else if (circular_mode == 0)
    {
      // Use linear fit only
      phiFit = phiFit_linear;
    }
    ...
    residual_rphi = residual_rphi/sqrt(3.0/2.0);// /sqrt(3.0/2.0); 
    // ============================================================
    // ERROR PROPAGATION
    // ============================================================
    ...
    // The prediction variance (for both linear and circular, first order):
    ...
    if (Verbosity() > 0)
    {
      std::cout << "ClusterPhaseAnalysis::ComputeCircularFitFromNeighbors -- ckey " << ckey
                << " layer " << currentLayer
                << "\n  Track: pT=" << pt << " GeV/c, charge=" << charge << ", κ=" << kappa << " cm⁻¹"
                ...
                << "\n  Linear fit: " << phiFit_linear << " rad"
                << "\n  Circular fit: " << phiFit << " rad (correction = " << (phiFit - phiFit_linear) << ")"
